...
module tcp_layer (
  ...
  // Instantiate the underlying TCP/IP stack
  ...
  // Main TCP layer logic
  always @(posedge clk) begin
    if (!rst_n) begin
      // Initialize TCP state and buffers
      ...
      // State machine for TCP connection
      case (tcp_state)
        TCP_STATE_CLOSED: begin
          if (app_tx_valid && app_tx_ready) begin
            // Start connection: send SYN packet
        ...
        TCP_STATE_SYN_SENT: begin
          if (ip_rx_valid && ip_rx_ready &&
              ip_rx_data[31:16] == {TCP_DST_PORT, TCP_SRC_PORT} &&
              ip_rx_data[15:0]  == 16'h5012) begin
        ...
        TCP_STATE_ESTABLISHED: begin
          if (app_tx_valid && app_tx_ready) begin
            // Send data packet to remote host
          ...
          if (ip_rx_valid && ip_rx_ready) begin
            if (ip_rx_data[31:16] == {TCP_DST_PORT, TCP_SRC_PORT} &&
                ip_rx_data[15:0]  == 16'h5011) begin
              // FIN-ACK received, move to FIN wait
              ...
              // Data packet received from remote host
        ...
        TCP_STATE_FIN_WAIT: begin
          if (app_rx_ready && app_rx_valid) begin
            // Application consumed FIN, close connection
            tcp_state <= TCP_STATE_CLOSED;
          end
        end
      endcase

      // Clear transmit buffer when packet is sent
      if (tx_buffer_valid && ip_tx_ready) begin
        tx_buffer_valid <= 0;
      end

      // Clear receive buffer when application consumes data
  ...
  // TODO: Implement optional TCP checksum calculation
