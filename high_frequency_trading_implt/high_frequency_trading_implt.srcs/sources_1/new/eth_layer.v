...
module eth_layer (
  ...
  // Main Ethernet layer logic
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      // Reset all buffers and handshake signals
      tx_length           <= 0;
      rx_length           <= 0;
      mac_tx_valid        <= 0;
      tcp_ip_rx_valid     <= 0;
      ...
      // Capture outgoing data from TCP/IP stack
      if (tcp_ip_tx_valid && tcp_ip_tx_ready) begin
        ...
        tx_length         <= 14 + 20 + tcp_ip_tx_data[31:16]; // Ethernet + IP header + payload
      end
